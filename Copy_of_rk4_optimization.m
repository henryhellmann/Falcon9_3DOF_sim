...
for iter = 1:max_iters
    ...
    for k = 1:N-1
        % 1. Get continuous linear Jacobians at the reference point
        [~, A, B, C] = get_linear_dyn(X_ref(:,k), U_ref(:,k), Gam_ref(k), p);
        
        % 2. TRUE NONLINEAR PROPAGATION (The Anchor)
        % We calculate where the reference trajectory would actually end up 
        % if we applied the exact nonlinear physics using your RK4 function.
        X_next_nl = RK4(0, X_ref(:,k), dt, U_ref(:,k), p);
        
        % 3. DISCRETE JACOBIANS
        % Forward Euler discretization for the *error* states
        Ad = eye(7) + A * dt;
        Bd = B * dt;
        Cd = C * dt;
        
        % 4. THE CORRECTED SCP CONSTRAINT
        % X_next = (True Nonlinear Base) + (Linearized Error)
        X_next = X_next_nl + Ad*(X(:,k) - X_ref(:,k)) + Bd*(U(:,k) - U_ref(:,k)) + Cd*(Gam(k) - Gam_ref(k));
        
        % YALMIP constraint with virtual control to ensure solver feasibility
        Constraints = [Constraints, X(:,k+1) == X_next + nu(:,k)];
