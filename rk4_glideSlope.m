...
for iter = 1:max_iters
    ...
    fprintf('Min altitude achieved: %.2f m\n', min(X_ref(3,:)));
    ...
    fprintf('Initial x: %.2f, Final x: %.2f\n', X_ref(1,1), X_ref(1,end));
fprintf('Initial z: %.2f, Final z: %.2f\n', X_ref(3,1), X_ref(3,end));
fprintf('Max x: %.2f, Max z: %.2f\n', max(X_ref(1,:)), max(X_ref(3,:)));
...
xlim([-50, 550]);
ylim([-50, 1100]);
legend('Glide-slope zone', 'Trajectory', 'Start', 'End');
